...
//nav - Função para abrir e fechar o menu hambúrguer
...
navLinks.forEach(link => {
    link.addEventListener("click", (event) => {
        ...
        }, 10);  // Delay de 500ms para garantir que a navegação ocorra antes do fechamento - Feito isso, pois estava ocorrendo um bug e não fechava o menu, e, quando fechava, não navegava corretamente até a seção alvo. 
    });
...
//hoverLink
// Seleciona todos os links com a classe .transitionLink
document.addEventListener('DOMContentLoaded', function () {
    // Seleciona os elementos
    const logo = document.getElementById('logoTransition'); // Seleciona a logo
    const links = document.querySelectorAll('.hoverLink'); // Seleciona todos os links com a classe .hoverLink
    const page = document.querySelector('body'); // Seleciona o corpo da página

    // Verifique se os elementos necessários existem antes de continuar
    if (!logo) {
        console.error("Elemento 'logoTransition' não encontrado.");
    }
    ...
    // Se algum dos elementos estiver ausente, não prossegue
    if (!logo || !page) {
        return; // Se qualquer um dos elementos não existir, interrompe a execução
    }

    console.log("Elementos necessários encontrados. Iniciando transição.");
    ...
    links.forEach(function (link) {
        link.addEventListener('click', function (event) {
            ...
            // Exibe a logo e aplica a transição de fade na página
            logo.classList.add('show'); // Mostra a logo
            page.classList.add('fadeOut'); // Aplica o fade na página

            // Após 500ms (tempo da transição), remove a logo e redireciona
            setTimeout(function () {
                logo.classList.remove('show'); // Remove a classe show para esconder a logo

                // Realiza o redirecionamento para a nova página
                window.location.href = link.getAttribute('href');
            }, 500); // 500ms para a animação de fade
        });
    });
});
